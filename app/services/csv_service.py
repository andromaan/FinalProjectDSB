...
async def fetch_exchange_rates() -> int | None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        usd_rate = next((item["rate"] for item in response.json() if item["cc"] == "USD"), None)
...
class CSVService:
    ...
    async def generate_scraped_cars_csv(self, cars_query: ScrapedCarQuery) -> io.BytesIO:
        cars = await self.scraping_repo.fetch_scraped_cars(
            id=cars_query.id,
            car_id=cars_query.car_id,
            request_id=cars_query.request_id,
            car_platform_id=cars_query.car_platform_id,
            date_of_scrape_from=cars_query.date_of_scrape_from,
            date_of_scrape_to=cars_query.date_of_scrape_to,
        )
        ...
        for car in cars:
            ...
            if car.scraped_currency in ["грн", "₴"]:
                price = round(car.scraped_price / exchange_rate) if exchange_rate else price
